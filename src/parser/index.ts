...
export function parse(input: string): ParseResult {
  ...
  if (diagramType === "unsupported") {
    return { ast: { type: "unsupported", detectedType: detectedType! }, warnings };
  }

  if (diagramType === "erd") {
    return { ast: parseERD(input, warnings), warnings };
  }
